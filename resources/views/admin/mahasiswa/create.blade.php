...
<div class="max-w-xl mx-auto glass-panel p-8 rounded-2xl shadow-xl">
    ...
    <form action="{{ route('admin.mahasiswa.store') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" id="uid_ktm" name="uid_ktm" value="{{ old('uid_ktm') }}" required placeholder="Contoh: A1B2C3D4"
        ...
        <div>
            ...
            <input type="text" id="nim" name="nim" value="{{ old('nim') }}" required placeholder="Masukkan NIM mahasiswa"
        ...
        <div>
            ...
            <input type="text" id="kelas" name="kelas" value="{{ old('kelas') }}" required placeholder="Contoh: 3A"
                   class="w-full bg-slate-900/60 border border-slate-800 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-all">
            @error('kelas')
                <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span>
            @enderror
        ...
        <div>
            ...
            <input type="text" id="prodi" name="prodi" value="{{ old('prodi') }}" required placeholder="Contoh: Teknik Informatika"
                   class="w-full bg-slate-900/60 border border-slate-800 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-all">
            @error('prodi')
                <span class="text-xs text-red-400 mt-1 block">{{ $message }}</span>
            @enderror
