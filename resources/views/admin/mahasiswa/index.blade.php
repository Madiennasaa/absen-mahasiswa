            ...
            <tbody class="divide-y divide-slate-800/40 text-sm">
                @forelse($mahasiswa as $mhs)
                    <tr class="hover:bg-slate-900/10 transition-colors">
                        <td class="py-3.5 px-4 font-mono text-indigo-400">{{ $mhs->uid_ktm }}</td>
                        ...
                        <td class="py-3.5 px-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.mahasiswa.edit', $mhs->uid_ktm) }}" class="p-2 text-indigo-400 hover:text-indigo-300 hover:bg-indigo-500/10 rounded-lg transition-all" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form action="{{ route('admin.mahasiswa.destroy', $mhs->uid_ktm) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus mahasiswa ini dan semua data absensinya?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-8 text-center text-slate-500">Tidak ada data mahasiswa.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
